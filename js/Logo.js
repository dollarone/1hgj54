...
PlatformerGame.Logo.prototype = {
  create: function() {
    ...
    this.game.stage.backgroundColor = "#f6f849";
...
// prod
    ...
    this.pressed = false;
    this.game.add.text(120, 100, "The Death Defying Unicorn", { fontSize: '48px Arial', fill: '#000' });

    this.game.add.text(130, 400, "     CRUSH the evil yellow unicorns.\n  If you let one through, you have lost,\nbecause it will destroy EVERYTHING!\n    (Spacebar to CRUSH; R to restart) ", { fontSize: '32px Arial', fill: '#000' });
    this.timer = 0;    
  },
  ...
  update: function() {
    ...
    if (this.timer % 3 == 0) {
        ...
        var offset = this.game.rnd.integerInRange(-1,1);
        var movex  = this.game.rnd.integerInRange(0,1);
        var movey  = this.game.rnd.integerInRange(0,1);


        this.platforms.forEach(function(tile) {
            if (movex == 0) {
                tile.x += offset;
            }
            if (movey == 0) {
